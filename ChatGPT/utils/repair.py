...
def repair_level_size(level):
    ...
    if height > desired_height:
        level = level[(height - desired_height):]
    elif height < desired_height:
        for _ in range(desired_height - height):
            level.insert(0, EMPTY * desired_width)
        
    for i in range(desired_height):
        if len(level[i]) > desired_width:
            level[i] = level[i][:desired_width]
        elif len(level[i]) < desired_width:
            for _ in range(desired_width - len(level[i])):
                level[i] += EMPTY
