...
for temp in possible_temperatures:
    # Make levels folder
    non_repaired_levels_folder = f"levels_MarioGPT_{temp}"
    os.makedirs(non_repaired_levels_folder, exist_ok=True)

    # Make repaired levels folder
    repaired_levels_folder = f"levels_MarioGPT_{temp}_repaired"
    ...
    for n in range(args.n_levels):
        ...
        generated_level = convert_level_to_vglc(generated_level.level)
        with open(f"{non_repaired_levels_folder}/level_p_{pipe}_e_{enemy}_b_{block}_h_{elevation}_n_{n}.txt", "w") as f:
