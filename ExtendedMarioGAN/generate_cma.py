...
parser.add_argument('--version', type=int, required=True, help='Version of the model to use. Please choose 1 (only trained on level 1-1) or 2 (trained on all levels)')
...
if args.version == 1:
    non_repaired_levels_folder = f"levels_CorrectedMarioGAN_v1"
    repaired_levels_folder = f"levels_CorrectedMarioGAN_v1_repaired"
    modelToLoad = "samples_v1/netG_epoch_5000.pth"
elif args.version == 2:
    non_repaired_levels_folder = f"levels_CorrectedMarioGAN_v2"
    repaired_levels_folder = f"levels_CorrectedMarioGAN_v2_repaired"
...
# Select all the fitness functions because CorrectedMarioGAN understands all tiles
fitness_functions = get_fitness_functions(generator, n_scenes_per_level, nz)
