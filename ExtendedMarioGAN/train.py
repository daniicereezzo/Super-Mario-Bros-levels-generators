...
# Create the output directory and set the training data file based on the version
if opt.version == 1:
    output_folder = "samples_v1"
    examplesJson = "training_data/first_level_scenes.json"
elif opt.version == 2:
    output_folder = "samples_v2"
    examplesJson= "training_data/all_scenes.json"
else:
    print("Invalid version. Please choose 1 (only trained on level 1-1) or 2 (trained on all levels). Use --help for more information.")
    sys.exit(1)
...
print ("SHAPE ",X_onehot.shape)    #(173, 14, 28, 13)
X_onehot = np.rollaxis(X_onehot, 3, 1)
print ("SHAPE ",X_onehot.shape)    #(173, 13, 28, 14)
...
if opt.version == 1:
    time_file = "training_time_v1.txt"
else:
    time_file = "training_time_v2.txt"
